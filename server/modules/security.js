let security = () => {
    ...
    Security.permit(['insert', 'update', 'remove']).collections([
        Cialcos, Organizaciones, Redes, Productores, MontosVenta, Metas, Respaldos, Uploads, CialcoIntegrantes, Meteor.users
    ]).ifHasRole('admin').apply();

    // Only if a user is logged in
    Security.permit(['insert']).collections([
      Cialcos, Organizaciones, Redes, Productores, MontosVenta, Metas, Respaldos, Uploads, CialcoIntegrantes
    ]).ifLoggedIn().apply();
